...
function GeoPoint(data) {
  ...
  assert(data.lat <= 180, 'lat must be <= 90'); // lat is actually longitude
  assert(data.lat >= -180, 'lat must be >= -90');
  assert(data.lng <= 90, 'lng must be <= 180'); // lng is actually latitude
  assert(data.lng >= -90, 'lng must be >= -180');
